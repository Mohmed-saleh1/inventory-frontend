...
export const OrderWidget = ({ order }) => {
  return (
    <div className="order-widget bg-gray-100 p-4 rounded-md shadow-md mb-4">
      ...
      <h4 className="text-sm text-gray-600 mb-2">Total Records: {order.record.length}</h4>
      ...
      <div className="overflow-x-auto">
        <table className="table-auto w-full border-collapse text-sm text-left">
          <thead className="bg-[#FAFAFA] text-gray-700">
            <tr>
              {["Name", "Image", "Price", "Description", "Amount","available", "sales", "waste"].map((col, index) => (
                <th
                  key={index}
          ...
          <tbody>
            {order.record.map((recordItem, index) => (
              <tr key={index} className="hover:bg-gray-50 font-manrope text-[#111827] text-[12px]">
                <td className="px-4 py-3">{recordItem.productId?.name || "N/A"}</td>
                ...
                <td className="px-4 py-3">{recordItem.productId?.description || "N/A"}</td>
                <td className="px-4 py-3">{recordItem.amount || "N/A"}</td>
                <td className="px-4 py-3">{recordItem.productId?.available || "N/A"}</td>
                <td className="px-4 py-3">{recordItem.productId?.sales || "N/A"}</td>
                <td className="px-4 py-3">{recordItem.productId?.waste || "N/A"}</td>
              </tr>
            ))}
          </tbody>
        </table>
